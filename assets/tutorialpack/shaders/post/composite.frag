...
void main() {
    ...
    Pair pairs[6];
    pairs[0] = Pair(texture(u_main_color, texcoord), texture(u_main_depth, texcoord).r);
    pairs[1] = Pair(texture(u_translucent_color, texcoord), texture(u_translucent_depth, texcoord).r);
    pairs[2] = Pair(texture(u_entity_color, texcoord), texture(u_entity_depth, texcoord).r);
    pairs[3] = Pair(texture(u_weather_color, texcoord), texture(u_weather_depth, texcoord).r);
    pairs[4] = Pair(texture(u_clouds_color, texcoord), texture(u_clouds_depth, texcoord).r);
    pairs[5] = Pair(texture(u_particles_color, texcoord), texture(u_particles_depth, texcoord).r);

    sort(pairs);

    vec3 composite = vec3(0.0);
    for (int i = 0; i < 6; i++) {
        composite = blend_colors(composite, pairs[i].color);
    }
